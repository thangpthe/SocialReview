...
namespace SocialReview.Controllers
{
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Detail(int id)
        {
            ...
            // Hiển thị TempData alerts từ redirect
        ...
        // BUG FIX: Trả về PartialView cho AJAX thay vì redirect
        // Nếu không phải AJAX (JS disabled) → vẫn redirect an toàn
        [Authorize]
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> CreateReview(
            [Bind(Prefix = "NewReviewForm")] CreateReviewViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    // Trả về PartialView cho product-detail.js xử lý
                    var user = await _userManager.GetUserAsync(User);
                    review.User = user;
                    return PartialView("~/Views/Shared/_ReviewCardPartial.cshtml", review);
                }

                // Fallback khi JS bị tắt
                TempData["SuccessMessage"] = "Cảm ơn bạn! Đánh giá đã được gửi thành công.";
                return RedirectToAction("Detail", new { id = model.ProductId });
            }
